...
const firebaseConfig = {
  apiKey: "AIzaSyBw5X0zwq2uErQs8FC3BfbTxOqU2ZeQLHw",
  authDomain: "tt-iot-w3.firebaseapp.com",
  databaseURL: "https://tt-iot-w3-default-rtdb.firebaseio.com",
  projectId: "tt-iot-w3",
  storageBucket: "tt-iot-w3.appspot.com",
  messagingSenderId: "722817295719",
  appId: "1:722817295719:web:51d20df44a1e9b55cafbc2",
  measurementId: "G-SZ5844JFDK"
};
...
function generatenhietdo() {
  var randomNumber = Math.floor(Math.random() * (25 - 22 + 1)) + 25;
 const allTag =  document.querySelectorAll(".generatenhietdo")
 allTag.forEach((tag)=>{
  tag.innerText = randomNumber
 })
}
generatenhietdo();
setInterval(generatenhietdo, 5000);

function generateRandomNumber() {
  // Generate a random number between 40 and 55
  var randomNumber = Math.floor(Math.random() * (40 - 37 + 1)) + 40;

  // Display the random number on the HTML page
 const allTag =  document.querySelectorAll(".generatehumid")
 allTag.forEach((tag)=>{
  tag.innerText = randomNumber
 })
}
...
function generategas() {
 ...
 const allTag =  document.querySelectorAll(".generategas")
 allTag.forEach((tag)=>{
  tag.innerText = randomNumber
 })
}
generategas();
setInterval(generategas, 1000);

function generatebright() {
  var randomNumber = Math.floor(Math.random() * (80 - 79 + 1)) + 80;
 const allTag =  document.querySelectorAll(".generatebright")
 allTag.forEach((tag)=>{
  tag.innerText = randomNumber
 })
}
generatebright();
setInterval(generatebright, 1000);
